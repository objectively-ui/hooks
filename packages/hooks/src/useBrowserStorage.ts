...
export interface UseBrowserStorageOptions<
	TData extends StorableRecord = StorableRecord,
> {
...
interface UseBrowserStorageReturn<TData extends StorableRecord> {
	data: Partial<TData>;
	setValue: <TKey extends string & keyof TData>(
		key: TKey,
		value: TData[TKey] | undefined,
	) => void;
...
export const useBrowserStorage = <TData extends StorableRecord>(
	...
	const {
		...
		deserializeValue = defaultDeserializer,
	} = opts;
