	...
	switch (value) {
		...
		default: {
			if (new RegExp(`^${ESCAPE_CHAR}-?\\d+(\\.\\d+)?$`).test(value)) {
				return parseFloat(value.slice(ESCAPE_CHAR.length));
			}
...
const encode = (json: JsonValue): string => {
	...
	if (typeof json === "object") {
		...
		let encodedObject = Object.entries(json)
			.map(([k, v]) => {
				if (typeof k !== "string") {
...
const decode = <T extends JsonValue = JsonValue>(encoded: string): T => {
	...
	if (encoded.startsWith(COLLECTION_START_CHAR)) {
		...
		// Heuristic: if there's a colon before a comma, treat as object, else array
		// Find first separator or colon
		const firstSep = inner.indexOf(SEPARATOR_CHAR);
		const firstColon = inner.indexOf(OBJECT_KV_CHAR);
